...
use crate::encoding::{decode_epub_text, TextKind};
...
pub fn replace_tag_block(source: &str, tag: &str, replacement: &str) -> Result<String, String> {
    let pattern =
        crate::backend::util::cached_regex(&format!(r"(?is)<{tag}\b[^>]*>.*?</{tag}\s*>"))
...
fn find_open_tag(source: &str, tag: &str) -> Option<String> {
    let pattern = crate::backend::util::cached_regex(&format!(r"(?is)<{tag}\b[^>]*>")).ok()?;
    pattern
        .find(source)
