...
fn rewrite_attribute(
    ...
    let pattern = crate::backend::util::cached_regex(&format!(
        r#"(?is)(<[^>]*\b{name}\s*=\s*[\"'])([^\"']*)([\"'][^>]*>)"#
    ))
...
mod tests {
    ...
    use crate::backend::epub::task_base::ManifestItem;
    ...
    fn keeps_stable_filename_algorithms() {
        let item = ManifestItem {
            ...
            resource_type: crate::backend::epub::task_base::ResourceType::Image,
        };
        assert!(
