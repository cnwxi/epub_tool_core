use crate::{
    backend::{epub::EpubWorkspace, EpubTask, TaskOutcome, TaskUpdate},
    encoding::{decode_epub_text, encode_epub_text, text_kind_for_path},
    task::{ChineseConversionDirection, TaskOptions, TaskType},
    EngineConfig,
};
...
fn converter(direction: &str, directory: &Path) -> Result<OpenccConverter, String> {
...
impl EpubTask for ChineseConvertTask {
    ...
    fn process(
        ...
        let directory = config
            .opencc_dir()
            .ok_or_else(|| "未提供 Rust OpenCC 词典资源目录".to_string())?;
        let converter = converter(direction.as_str(), directory)?;
...
mod tests {
    ...
    use crate::backend::EpubTask;
    use crate::task::{ChineseConversionDirection, TaskOptions};
    use std::path::PathBuf;

    fn fixture_opencc_dir() -> PathBuf {
        PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("tests/fixtures/opencc")
    }
    ...
    fn converts_visible_text_and_attributes_without_touching_script_or_css() {
        let converter = converter("s2t", &fixture_opencc_dir()).unwrap();
    ...
    fn bundled_dictionary_matches_opencc_t2s_phrase_vectors() {
        let converter = converter("t2s", &fixture_opencc_dir()).unwrap();
