...
use crate::{
    task::{FileIssue, TaskEvent, TaskOptions, TaskResult, TaskSpec, TaskSummary, TaskType},
    EngineConfig,
};
...
pub(crate) fn supports(request: &TaskSpec) -> bool {
...
fn run_file(
    ...
    let outcome = task.process(input, &mut workspace, options, config, &mut update)?;
    take_update_error(&update_error)?;
...
fn task_for(task_type: TaskType) -> Option<Box<dyn EpubTask>> {
    match task_type {
        ...
        TaskType::EncryptFont | TaskType::DecryptFont => None,
...
mod tests {
    ...
    use crate::{
        task::{FontTaskOptions, ImageTaskOptions, TaskOptions, TaskSpec, TaskType},
        EngineConfig,
    };
    ...
    fn typed_file_updates_emit_monotonic_overall_task_progress() {
        ...
        let config = EngineConfig::new(&log_path);
    ...
    fn font_capability_probe_leaves_epub_validation_to_each_file() {
        ...
        assert_eq!(supports(&request), cfg!(feature = "font"));
    }
    ...
    fn native_image_task_preserves_frontend_result_and_event_schema() {
        ...
        let result = run(&request, &config, &mut |event| {
            events.push(event);
            Ok(())
        })
        .unwrap();
