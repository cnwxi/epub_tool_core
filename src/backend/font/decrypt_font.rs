...
use crate::{
    ...
    encoding::{decode_epub_text, encode_epub_text, text_kind_for_path, TextKind},
    task::{OcrCharPolicy, TaskOptions, TaskType},
    EngineConfig,
};
...
impl EpubTask for DecryptFontTask {
    ...
    fn process(
        ...
        let resources = ocr_resources(config)?;
...
struct OcrResourcePaths {
    model_dir: PathBuf,
}

fn ocr_resources(config: &EngineConfig) -> Result<OcrResourcePaths, String> {
    let model_dir = config
        .ocr_model_dir()
        .ok_or_else(|| "未提供 Rust OCR 所需的 ONNX Runtime 或 PP-OCR 模型资源目录".to_string())?;
    if !model_dir.join("inference.onnx").is_file() || !model_dir.join("inference.yml").is_file() {
        return Err(format!("OCR 模型资源不完整: {}", model_dir.display()));
    }
    Ok(OcrResourcePaths {
...
mod tests {
    ...
    fn parses_bundled_paddleocr_config() {
        let config =
            parse_ocr_model_config(include_str!("../../../tests/fixtures/ocr/inference.yml"))
