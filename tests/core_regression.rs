use epub_tool_core::{
    run, supports, ChineseConversionDirection, EngineConfig, EngineError, ImageTaskOptions,
...
fn run_task(
    ...
    let config = EngineConfig::new(directory.join(format!("{task_id}.log")))
        .with_opencc_dir(opencc_fixture_dir());
